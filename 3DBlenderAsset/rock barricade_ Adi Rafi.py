...
def get_or_create_collection(name):
    """Bikin collection baru, atau pakai yang sudah ada, supaya rapi di outliner."""
...
def clear_collection(coll):
    """Bersihkan semua objek lama di collection sebelum generate ulang."""
    for obj in list(coll.objects):
        bpy.data.objects.remove(obj, do_unlink=True)

def create_realistic_sandbag_material():
    """Material prosedural karung pasir militer: hijau tua usang, belang
    pudar akibat cuaca, kotor di lipatan/celah, plus bercak bekas terbakar
    / lubang tembakan (voronoi) supaya terkesan bekas peperangan.
    Kalau node graph gagal dibuat (beda versi Blender), fallback ke
    warna solid hijau tua supaya script tetap jalan."""
    ...
    try:
        ...
        # noise kecil -> butiran kain kasar & kotoran halus
        ...
        ramp_dirt.color_ramp.elements[0].color = (0.035, 0.03, 0.02, 1.0)   # kotoran coklat gelap
        ...
        ramp_scorch.color_ramp.elements[0].color = (0.015, 0.015, 0.015, 1.0)  # hitam bekas terbakar
...
def bake_material_to_texture(obj, mat, filename, size=TEXTURE_SIZE):
    """Bake material prosedural jadi 1 file texture gambar (.png), lalu
    sambungkan ulang Base Color BSDF ke gambar hasil bake itu (bukan node
    prosedural lagi). Ini WAJIB supaya texture ikut kebawa saat export FBX,
    karena Roblox Studio tidak mengerti node shader Blender -- cuma file
    gambar texture biasa yang bisa dipakai (lewat SurfaceAppearance/Decal)."""
    try:
...
def apply_smart_shading(obj, angle_deg=42):
    """Smooth shading HANYA di permukaan yang landai (sudut kecil), supaya
    siluet kotak/kantung karung tetap kelihatan -- bukan dihaluskan total
    jadi bentuk bulat seperti kerikil/batu. Kompatibel beberapa versi Blender."""
    bpy.ops.object.select_all(action='DESELECT')
    obj.select_set(True)
    bpy.context.view_layer.objects.active = obj
    try:
...
def set_origin_to_base(obj):
    """Pindahkan origin/pivot objek ke bagian bawah-tengah (base),
    supaya gampang di-place di atas ground saat dipakai di Roblox Studio."""
...
def create_sandbag_mesh(name, bag_seed, coll):
    """Bikin 1 mesh karung pasir organik: cube -> subdivide -> tapering
    di ujung supaya bentuknya seperti kantung, lalu ditambah noise
    acak di permukaan supaya tidak terlihat seperti kotak sempurna."""
    bm = bmesh.new()
...
def create_spilled_sand(location, severity, rng, coll):
    """Bikin gundukan pasir tumpah kecil (untuk karung yang sobek/pecah)."""
...
def displace_damaged_bag(obj, severity, rng):
    """Geser & miringkan karung yang 'rusak' seolah terkena hantaman/serangan.
    severity 0-1: makin besar, makin jauh bergeser / makin ekstrim rotasinya."""
...
def build_barricade_variant(variant_name, damage_level, origin_offset, seed, coll):
    """damage_level 0.0 = rapi utuh, 1.0 = hancur total & berantakan."""
    ...
    for row in range(ROWS):
        # piramida: baris makin ke atas makin pendek (mengecil dari kedua sisi)
        ...
        for i in range(row_count):
            ...
            # sesekali ada karung "header" diputar 90 derajat untuk variasi visual
            if rng.random() < 0.15:
...
def finalize_variant(objects, variant_name, material):
    """Gabungkan semua karung jadi 1 objek, UV unwrap, set origin ke base,
    scale ke satuan stud Roblox, apply transform, lalu export FBX."""
    bpy.ops.object.select_all(action='DESELECT')
    for obj in objects:
        obj.select_set(True)
    ...
    # UV unwrap otomatis, siap untuk texture painting nanti
    bpy.ops.object.mode_set(mode='EDIT')
    bpy.ops.mesh.select_all(action='SELECT')
